...
| Primary button | `bg-[#01aa3f]`, hover `#00ff59` + `text-black` flip | CTAs, header "Request a Quote" |
| Error text | `#c62828` | form validation messages (WCAG AA on white) |
| Accent | `#00c853` | mono eyebrows, check icons, type labels, links, focus borders, active nav pill (`bg-[#01aa3f]/12 text-[#01aa3f]`) |
...
- **Primary pill:** `rounded-full bg-[#01aa3f] px-7 py-3.5 text-sm font-medium text-black transition-all hover:-translate-y-px hover:bg-[#00ff59] hover:text-black active:scale-[0.98]` + trailing `ArrowUpRight className="size-4"`.
- **Ghost pill (over photo):** `rounded-full border border-white/60 px-7 py-3.5 text-sm text-white drop-shadow-md transition-colors hover:border-white/80 hover:bg-white/10 active:scale-[0.98]`.
...
- **Hero category card (marquee):** photo-only tile (no caption) — `w-[120px] aspect-[3/4]` up to `xl:w-[220px]`, `rounded-2xl border border-black/10`, `active:scale-[0.98]`, `aria-label="View <category> products"`. **Single full-bleed row:** all 10 categories drift in one direction; the set is rendered 2x per half inside a CSS marquee (`data-marquee` / `data-marquee-track`, translateX 0→-50% at 60s, soft edge mask, paused on hover/focus, `prefers-reduced-motion` → static scrollable row); tiles are `<Link href="/showroom?filter=<category>">` using the same compressed showroom photos; defined in `lib/showroom.ts` `HERO_MARQUEE` (merged from `HERO_CATEGORIES` + `HERO_CATEGORIES_SECONDARY`).
...
- `transition-colors` on links/buttons; `hover:-translate-y-px` on primary CTAs; `hover:-translate-y-1` on grid cards; image zoom `duration-500`; mobile menu slide/fade `duration-500 ease-in-out` (always-mounted + `will-change-transform` — mount/unmount would kill the exit animation); `active:scale-[0.98]`/`[0.95]` press state on pills, chips, and icon buttons; menu/hover colors flip only. Nothing else.
- **Reduced motion:** `globals.css` includes a `@media (prefers-reduced-motion: reduce)` block that collapses all transitions/animations.

## Global CSS (in `app/globals.css`)

- `:focus-visible` → `outline: 2px solid #00c853; outline-offset: 2px` (keyboard-only green ring).
- `body::after` film-grain overlay (fixed, `z-index: 60`, `pointer-events: none`, ~3% opacity, SVG noise data-URI) — adds texture to flat light surfaces; never intercepts input.
...
- `components/hero-section.tsx` / `lib/showroom.ts` — hero: ≈90vh band (backdrop `h-[90vh]`, section `min-h-[calc(90vh-5.25rem)] md:min-h-[calc(90vh-4.375rem)]`, content vertically centered) — **one full-bleed single-row marquee** of all 10 photo-only category cards (`HERO_MARQUEE`), then ghost + green CTAs (→ `/showroom` / `/quote`) below; no visible heading — a `sr-only` h1 carries the page title; visual reference for every other page.
- `components/showroom-section.tsx` product data + showroom grid of name-only, center-aligned cards (`text-[13px]` name only; photo-only tile with hover zoom; no description, no pill, no eyebrow - the whole card is one `<button>` with green focus-visible ring); filter chips & product clicks sync the URL (`?filter=` / `?product=`) via pushState + popstate; `components/showroom-detail.tsx` - product detail: 5 gallery views, technical-specs table directly under the compact title, "Customization options" as a click-to-open drawer (`aria-expanded`/`aria-controls`, chevron rotate, `grid-rows` transition, closed by default), spacing tightened ~40%.
- `components/factory-page.tsx` / `components/standards-page.tsx` — `/factory` and `/standards`, sharing `components/page-shell.tsx` (light shell — legacy `DarkShell` merged into one); `components/certificate-gallery.tsx` — certificate lightbox.
- `app/quote/page.tsx` + `components/quote-form.tsx` — compact single-screen quote form (logo + wordmark + close button over the photo, no site header; visible page heading removed by design, sr-only h1 kept for screen readers/SEO).
