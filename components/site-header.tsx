...
export function SiteHeader() {
  ...
  return <>
    <header className={`relative z-30 mx-auto flex w-full max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 ${isDark ? 'text-white' : 'text-[#17201e]'}`}>
      <a href="/" onClick={() => setIsOpen(false)} className="flex items-center gap-2" aria-label="Home"><span className="flex size-9 items-center justify-center rounded-lg bg-emerald-500 text-black"><svg aria-hidden="true" viewBox="0 0 20 20" className="size-5" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M5 3h7a5 5 0 0 1 5 5v4a5 5 0 0 1-5 5H8l-3 1v-4a5 5 0 0 1-2-4V8a5 5 0 0 1 5-5Z" /></svg></span><span className="text-sm font-semibold tracking-tight">Trims Co.</span></a>
      ...
      <div className="flex items-center gap-2"><a href="/quote" className="hidden rounded-full bg-emerald-500 px-5 py-2.5 text-sm font-medium text-black transition-colors hover:bg-emerald-400 sm:inline-flex">Request a Quote</a><button type="button" aria-label={isOpen ? 'Close menu' : 'Open menu'} aria-expanded={isOpen} onClick={() => setIsOpen(!isOpen)} className={`inline-flex size-10 items-center justify-center rounded-full border backdrop-blur-sm md:hidden ${isDark ? 'border-white/15 bg-white/5 text-white' : 'border-[#d8e1e0] bg-white text-[#17201e]'}`}>{isOpen ? <X className="size-5" /> : <Menu className="size-5" />}</button></div>
    </header>
