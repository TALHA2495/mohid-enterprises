...
export function SiteHeader() {
  ...
  return <>
    <header className="relative z-30 mx-auto flex w-full items-center justify-between gap-4 bg-white border-b border-black/10 px-4 py-5 text-black sm:px-6">
      <Link href="/" onClick={() => setIsOpen(false)} className="flex items-center gap-2.5" aria-label="Home"><Image src={LOGO_URL} alt="Mohid Enterprises logo" width={1600} height={1491} sizes="(min-width: 768px) 24px, 32px" className="h-8 w-auto object-contain md:h-6" priority /><span className="flex flex-col leading-[1.15]"><span className="text-[16px] font-bold tracking-[0.055em]">MOHID</span><span className="text-[12px] font-semibold tracking-[0.12em] text-current opacity-75">ENTERPRISES</span></span></Link>
      ...
      <div className="flex items-center gap-2"><Link href="/quote" className="hidden rounded-full bg-[#01aa3f] px-5 py-2.5 text-sm font-medium text-black transition-all hover:-translate-y-px hover:bg-[#00ff59] hover:text-black active:scale-[0.98] sm:inline-flex">Request a Quote</Link><button type="button" aria-label={isOpen ? 'Close menu' : 'Open menu'} aria-expanded={isOpen} onClick={() => setIsOpen(!isOpen)} className="inline-flex size-11 items-center justify-center rounded-full border border-black/15 bg-black/[0.04] text-black backdrop-blur-sm transition-colors active:scale-[0.95] md:hidden">{isOpen ? <X className="size-5" /> : <Menu className="size-5" />}</button></div>
    </header>
