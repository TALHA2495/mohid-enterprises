...
export function QuoteForm() {
  ...
  return <form onSubmit={(e) => { e.preventDefault(); setSent(true) }} className="grid gap-4">
    <label className="grid gap-1 text-xs font-medium text-white/80">Inquiry details<textarea required rows={4} placeholder="Tell us about your material, width, color, or trimming specifications..." className="mt-1 resize-none rounded-lg border border-white/10 bg-white/[0.04] p-3 text-sm font-normal text-white outline-none placeholder:text-white/35 focus:border-[#00c853]" /></label>
    ...
    <label className="grid gap-1 text-xs font-medium text-white/80">Additional notes<textarea rows={3} className="resize-none rounded-lg border border-white/10 bg-white/[0.04] p-3 text-sm font-normal text-white outline-none focus:border-[#00c853]" /></label>
